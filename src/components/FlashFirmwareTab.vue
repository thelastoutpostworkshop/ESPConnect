<template>
    ...
    <v-card-text class="tools-card__body">
      ...
      <div v-if="partitionOptions.length" class="partition-tools">
        <v-select :model-value="selectedPartition" :items="partitionOptions" item-title="label" item-value="value"
          :label="$t('flash.partition')" density="comfortable" clearable :disabled="busy || maintenanceBusy"
      ...
      <v-row dense>
        <v-col cols="12" md="6">
          <v-text-field :model-value="flashReadOffset" :label="$t('flash.startOffset')" placeholder="0x0" density="comfortable"
            :disabled="busy || maintenanceBusy" @update:model-value="value => emit('update:flashReadOffset', value)" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field :model-value="flashReadLength" :label="$t('flash.lengthBytes')" placeholder="0x100000"
  ...
  <v-card class="tools-card" variant="tonal" prepend-icon="mdi-lightning-bolt">
    <template v-slot:title>
      <span class="font-weight-black"> {{ $t('flash.firmwareTitle') }}</span>
    </template>
    <v-card-text class="tools-card__body">
      <v-row class="mb-2" dense>
        <v-col cols="12" md="8">
          <v-file-input :label="$t('flash.firmwareBinary')" prepend-icon="mdi-file-upload" accept=".bin"
            density="comfortable" :disabled="busy || maintenanceBusy"
            @update:model-value="value => emit('firmware-input', value)" />
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field :model-value="flashOffset" :label="$t('flash.flashOffset')" placeholder="0x0" density="comfortable"
            :disabled="busy || maintenanceBusy" @update:model-value="value => emit('update:flashOffset', value)" />
        </v-col>
        <v-col cols="12" md="4">
          <v-select :model-value="selectedPreset" :items="offsetPresets" :label="$t('flash.recommendedOffsets')" item-title="label"
      ...
      <v-checkbox :model-value="eraseFlash" :label="$t('flash.eraseBeforeWrite')" density="comfortable" hide-details
  ...
  <v-card class="tools-card mt-6" variant="tonal" prepend-icon="mdi-chip">
    <template v-slot:title>
      <span class="font-weight-black"> {{ $t('flash.registerTitle') }}</span>
    </template>
    <v-card-text class="tools-card__body">
      <v-autocomplete v-if="registerOptions.length" class="register-quick-select" :items="registerOptions"
        ...
        :label="$t('flash.quickSelect')" :model-value="selectedRegisterAddress" :return-object="false" clearable
      ...
      <v-alert v-if="selectedRegisterInfo" type="info" variant="tonal" border="start" density="comfortable"
        ...
        <div class="register-info__link" v-if="selectedRegisterInfo.link">
          <a :href="selectedRegisterInfo.link" target="_blank" rel="noopener">{{ $t('flash.viewRegisterReference') }}</a>
        </div>
      </v-alert>
      <v-alert v-else-if="registerReference" type="info" variant="tonal" border="start" density="comfortable"
        ...
        <div class="register-info__link">
          <a :href="registerReference.url" target="_blank" rel="noopener">{{ $t('flash.openTechnicalReference') }}</a>
        </div>
      </v-alert>
      <v-row dense>
        <v-col cols="12" md="6">
          <v-text-field :model-value="registerAddress" :label="$t('flash.registerAddress')" placeholder="0x60000000"
            density="comfortable" :disabled="busy || maintenanceBusy"
            @update:model-value="value => emit('update:registerAddress', value)" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field :model-value="registerValue" :label="$t('flash.registerValue')" placeholder="0x0" density="comfortable"
  ...
  <v-card class="tools-card mt-6" variant="tonal" prepend-icon="mdi-shield-check-outline">
    <template v-slot:title>
      <span class="font-weight-black"> {{ $t('flash.flashIntegrityTitle') }}</span>
    </template>
    <v-card-text class="tools-card__body">
      <v-select v-if="partitionOptions.length" class="integrity-select" :items="partitionOptions" item-title="label"
        item-value="value" variant="outlined" density="comfortable" clearable :label="$t('flash.partition')"
      ...
      <p v-if="partitionOptions.length" class="integrity-helper">
        {{ $t('flash.selectPartitionHelper') }}
      </p>
      <v-row dense class="flash-progress-row">
        <v-col cols="12" md="6">
          <v-text-field :model-value="md5Offset" :label="$t('flash.startOffset')" placeholder="0x0" density="comfortable"
            :disabled="busy || maintenanceBusy" @update:model-value="value => emit('update:md5Offset', value)" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field :model-value="md5Length" :label="$t('flash.lengthBytes')" placeholder="0x100000" density="comfortable"
      ...
      <v-alert v-else-if="md5Result" type="success" variant="tonal" density="comfortable" border="start" class="mt-3">
        {{ $t('flash.md5Checksum') }} <code>{{ md5Result }}</code>
      </v-alert>
    </v-card-text>
  </v-card>
